...
напиши /add_film , чтобы добавить фильм
"""
...
def search_film(query):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM films WHERE title LIKE ? OR director LIKE ? OR genre LIKE ?", 
...
async def add_film_handler(message: types.Message):
    ...
    if user_input:
        ...
        if film:
            ...
            keyboard.add(InlineKeyboardButton("Добавить", callback_data=f"add_{film[0]}"))
            keyboard.add(InlineKeyboardButton("Отказаться", callback_data="cancel"))
            await message.reply(f"Найден фильм: {film[1]} - {film[2]} ({film[3]})", reply_markup=keyboard)
        else:
...
async def add_film_callback_handler(call: types.CallbackQuery):
    ...
    if film:
        ...
        keyboard = InlineKeyboardMarkup()
        keyboard.add(InlineKeyboardButton("Показать мои фильмы", callback_data="show_films"))
        keyboard.add(InlineKeyboardButton("Добавить еще фильм", callback_data="add_another"))

        await call.message.reply(f"Фильм {film[1]} - {film[2]} ({film[3]}) добавлен!", reply_markup=keyboard)
    else:
...
async def recommend_handler(message: types.Message):
    ...
    if user_films:
        ...
        if recommended_films:
            ...
            await message.reply("К сожалению, нет новых рекомендаций по вашим жанрам. Попробуйте добавить больше фильмов!")
    else:
        await message.reply("Вы пока не добавили ни одного фильма. Добавьте хотя бы один, чтобы получить рекомендации.")
