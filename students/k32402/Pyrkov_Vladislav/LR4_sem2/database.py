...
def find_film(query):
    conn = create_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM films WHERE title LIKE ? OR director LIKE ?", (f"%{query}%", f"%{query}%"))
...
def add_user(user_id, full_name):
    conn = create_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT OR IGNORE INTO users (id, full_name, films_watched) VALUES (?, ?, ?)", (user_id, full_name, 0))
    conn.commit()
    conn.close()
...
def add_film_to_user(user_id, film_id):
    conn = create_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO user_films (user_id, film_id) VALUES (?, ?)", (user_id, film_id))
    conn.commit()
    conn.close()
